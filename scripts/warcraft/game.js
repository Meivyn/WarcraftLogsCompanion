...
currentOverwolfGameId() {
	return this._version === 'classic' ? 21630 : 765;
}
...
origin() { return this.scheme() + "://" + this._host; }
...
initialRegion() {
	return this._version == "classic" ? 6 : 1;
}

regions() {
	return this._version === "classic" ? undefined : this._liveRegions;
}

versions() { return { "live": "live_version", "classic": "classic_version", "test": "test_version" }; }

modifyVersionForLanguage(version, language) { return version; }

setLanguageAndVersion(langService, version) {
	this._langService = langService
	this._lang = lang.language();
	this._version = version;
	if (this._lang == "en" && this._version == "classic")
		this._host = "classic.warcraftlogs.com";
	else
		this._host = (this._lang == "en" ? "www" : this._lang) + (this._version == "classic" ? ".classic" : "") + ".warcraftlogs.com";
}
...
versionForExecutionPath(executionPath)
{
	if (executionPath.indexOf('_classic_') != -1)
		return "classic";
	if (executionPath.indexOf('_ptr_') != -1 || executionPath.indexOf('_beta_') != -1)
		return "test";
	return "live";
}
...
supportsGear() {
	return this._version != 'classic' && this._lang != "cn";
}
...
nameForDifficulty(difficulty, logVersion) {
	...
	if (difficulty == 14)
		return logVersion == 1 ? this._langService.trans("warcraft_difficulty_flex") : this._langService.trans("warcraft_difficulty_normal")
	if (difficulty == 15)
		return this._langService.trans("warcraft_difficulty_heroic")
	if (difficulty == 200)
		return this._langService.trans("warcraft_difficulty_tower")
	if (difficulty == 8)
		return logVersion >= 7 ? this._langService.trans("warcraft_difficulty_mythic_plus") : this._langService.trans("warcraft_difficulty_challenge_mode")
...
desktopContentLoaded(doc) {
	...
	titleIfText.innerHTML = '<span id="title-top"><span id="title-top-first">WARCRAFT</span> <span id="title-top-second">LOGS </span></span>';
}

adjustTimeForYearCrossingAndDaylightSavings(line, dateString, timeSplit)
{
	if (!window._splitYearSet)
		window._splitYear = new Date().getFullYear()

	var date = new Date(dateString + "/" + window._splitYear + " " + timeSplit[0])
	...
	if (!window._splitYearSet) {
		while ((date.getTime() - new Date().getTime()) > 0) {
			...
			window._splitYear--;
			date = new Date(dateString + "/" + window._splitYear + " " + timeSplit[0])
			date.setMilliseconds(timeSplit[1])
		}
	...
	if (currTime >= window._previousSplitTime)
		return currTime

	date = new Date(dateString + "/" + (window._splitYear + 1) + " " + timeSplit[0])
	date.setMilliseconds(timeSplit[1])
	if (date.getTime()  - new Date().getTime() <= 0) {
		...
		while (currTime + window._daylightSavingsSplitShift < window._previousSplitTime)
...
scanLogLine(line)
{
	...
	const timeStringWithMilliseconds = line.substr(dateEnd + 1, timeEnd - dateEnd - 1)
	...
	const currTime = game.adjustTimeForYearCrossingAndDaylightSavings(line, dateString, timeSplit)
...
handleGameInfoUpdate(data, triggerEventCallback) {
	if (this._version === "classic") return;
	...
	if (groupApplicantsJsonString !== undefined) {
		this.handleGroupApplicantsGameInfoUpdate(groupApplicantsJsonString, triggerEventCallback);
	}
}

async handleGroupApplicantsGameInfoUpdate(groupApplicantsJsonString, triggerEventCallback) {
	...
	let groupApplicants = groupApplicantKeys.map(key => {
	...
	triggerEventCallback('groupApplicantsUpdated', {
		groupApplicants: window.sanitization.sanitizeHtmlStringsInJson(groupApplicants)
	});
}

async enrichGroupApplicants(groupApplicants) {
	if (!groupApplicants.length)
		return [];

	this._groupApplicantsExtraDataCache = this._groupApplicantsExtraDataCache || {};
	...
	const buildCacheKey = groupApplicant => `${groupApplicant.playerName}-${groupApplicant.serverSlug}-${groupApplicant.regionName}`;
	...
	if (!searchCriteria.characters.length)
		return groupApplicants;

	const response = await fetch(this.origin() + '/character/search-for-scores', {
		method: 'post',
	...
	const extraDatas = window.sanitization.sanitizeHtmlStringsInJson(await response.json());
	if (!extraDatas)
		return groupApplicants;

	groupApplicants.forEach(groupApplicant => {
		const extraData = extraDatas.find(x => x.character.name === groupApplicant.playerName && x.character.server.normalizedName === groupApplicant.serverSlug);
		if (!extraData)
			return;

		this._groupApplicantsExtraDataCache[buildCacheKey(groupApplicant)] = extraData;
