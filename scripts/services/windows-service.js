define([
  ...
  /**
   * restore or minimize a window by name
   * @param name
   * @returns {Promise<any>}
   */
  async function restoreOrMinimize(name) {
    await obtainWindow(name);
    const state = await getWindowState(name);

    if (state !== 'normal') {
      return await restore(name);
    }

    return await minimize(name);
  }

  /**
   * minimizes a window if it is open
   * @param name
   * @returns {Promise<any>}
   */
  async function minimizeIfOpen(name) {
    await obtainWindow(name);
    const state = await getWindowState(name);
  ...
  function setTopmost(windowId, topmost) {
    return new Promise(async (resolve, reject) => {
      try {
        overwolf.windows.setTopmost(windowId, topmost, (result) => {
          if (result.status === 'success') {
            resolve();
          } else {
            reject(result);
  ...
  function bringToFront(windowId, grabFocus) {
    return new Promise(async (resolve, reject) => {
      try {
