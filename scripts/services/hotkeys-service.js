define(['../constants/hotkeys-ids.js'], function (HOTKEYS) {
  ...
  function getToggleHotkey() {
    return new Promise((resolve, reject) => {
      _getHotkey(HOTKEYS.TOGGLE_VISIBILITY, function (result) {
        resolve(result);
      });
    });
  }
  ...
  function getCallWipeHotkey() {
    return new Promise((resolve, reject) => {
      _getHotkey(HOTKEYS.CALL_WIPE, function (result) {
        resolve(result);
      });
    });
  }

  function setCallWipeHotkey(action) {
    _setHotkey(HOTKEYS.CALL_WIPE, action);
  }
